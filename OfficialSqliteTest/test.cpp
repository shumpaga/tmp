...
"  --shrink-memory     Invoke sqlite3_a3d_db_release_memory() frequently.\n"
...
# define sqlite3_a3d_int64 sqlite_int64
#endif

typedef sqlite3_a3d_uint64 u64;
...
static struct Global {
    ...
    sqlite3_a3d_stmt* pStmt;       /* Current SQL statement */
    sqlite3_a3d_int64 iStart;      /* Start-time for the current test */
    sqlite3_a3d_int64 iTotal;      /* Total time */
    int bWithoutRowid;         /* True for --without-rowid */
    int bReprepare;            /* True to reprepare the SQL on each rerun */
    int bSqlOnly;              /* True to print the SQL once only */
    int bExplain;              /* Print SQL with EXPLAIN prefix */
    int bVerify;               /* Try to verify that results are correct */
    int bMemShrink;            /* Call sqlite3_a3d_db_release_memory() often */
...
/* Provide an alternative to sqlite3_a3d_stricmp() in older versions of
** SQLite */
#if SQLITE_VERSION_NUMBER<3007011
# define sqlite3_a3d_stricmp strcmp
#endif
...
static int integerValue(const char* zArg) {
    ...
    for (i = 0; i < sizeof(aMult) / sizeof(aMult[0]); i++) {
        if (sqlite3_a3d_stricmp(aMult[i].zSuffix, zArg) == 0) {
            v *= aMult[i].iMult;
            break;
        }
    }
...
sqlite3_a3d_int64 speedtest1_timestamp(void) {
#if SQLITE_VERSION_NUMBER<3005000
    return 0;
#else
    static sqlite3_a3d_vfs* clockVfs = 0;
    sqlite3_a3d_int64 t;
    if (clockVfs == 0) clockVfs = sqlite3_a3d_vfs_find(0);
...
#endif
    {
        ...
        t = (sqlite3_a3d_int64)(r * 86400000.0);
    }
    return t;
#endif
}
...
** The returned string should be freed with sqlite3_a3d_free().
**
** Example:
**
...
int speedtest1_numbername(unsigned int n, char* zOut, int nOut) {
    ...
    if (n >= 1000000000) {
        ...
        sqlite3_a3d_snprintf(nOut - i, zOut + i, " billion");
        i += (int)strlen(zOut + i);
        n = n % 1000000000;
    }
    if (n >= 1000000) {
        ...
        sqlite3_a3d_snprintf(nOut - i, zOut + i, " million");
        i += (int)strlen(zOut + i);
        n = n % 1000000;
    }
    if (n >= 1000) {
        ...
        sqlite3_a3d_snprintf(nOut - i, zOut + i, " thousand");
        i += (int)strlen(zOut + i);
        n = n % 1000;
    }
    if (n >= 100) {
        if (i && i < nOut - 1) zOut[i++] = ' ';
        sqlite3_a3d_snprintf(nOut - i, zOut + i, "%s hundred", ones[n / 100]);
        i += (int)strlen(zOut + i);
        n = n % 100;
    }
    if (n >= 20) {
        if (i && i < nOut - 1) zOut[i++] = ' ';
        sqlite3_a3d_snprintf(nOut - i, zOut + i, "%s", tens[n / 10]);
        i += (int)strlen(zOut + i);
        n = n % 10;
    }
    if (n > 0) {
        if (i && i < nOut - 1) zOut[i++] = ' ';
        sqlite3_a3d_snprintf(nOut - i, zOut + i, "%s", ones[n]);
        i += (int)strlen(zOut + i);
    }
    if (i == 0) {
        sqlite3_a3d_snprintf(nOut - i, zOut + i, "zero");
        i += (int)strlen(zOut + i);
    }
    return i;
}
...
void speedtest1_begin_test(int iTestNum, const char* zTestName, ...) {
    ...
    zName = sqlite3_a3d_vmprintf(zTestName, ap);
    va_end(ap);
    n = (int)strlen(zName);
    if (n > NAMEWIDTH) {
        zName[NAMEWIDTH] = 0;
...
void speedtest1_end_test(void) {
    sqlite3_a3d_int64 iElapseTime = speedtest1_timestamp() - g.iStart;
...
static void printSql(const char* zSql) {
    ...
    if (g.bExplain
        ...
        && (sqlite3_a3d_strglob("CREATE *", zSql) == 0
            || sqlite3_a3d_strglob("DROP *", zSql) == 0
            || sqlite3_a3d_strglob("ALTER *", zSql) == 0
            )
#endif
        ) {
        printf("%.*s;\n", n, zSql);
...
void speedtest1_shrink_memory(void) {
    ...
    if (g.bMemShrink) sqlite3_a3d_db_release_memory(g.db);
#endif
}

/* Run SQL */
A3D::SqliteWrapper wrapper("database.db");
void speedtest1_exec(const char* zFormat, ...) {
    va_list ap;
    char* zSql;
    va_start(ap, zFormat);
    zSql = sqlite3_a3d_vmprintf(zFormat, ap);
    va_end(ap);
    if (g.bSqlOnly) {
        printSql(zSql);
    }
    else {
        if (!wrapper.ExecStatement(zSql))
        {
            fatal_error("exec error: %s\n", sqlite3_a3d_errmsg(g.db));
        }
        else
        {
            //printf("Exec Success\n");
...
char* speedtest1_once(const char* zFormat, ...) {
    ...
    if (g.bSqlOnly) {
        ...
        int rc = sqlite3_a3d_prepare_v2(g.db, zSql, -1, &pStmt, 0);
        if (rc) {
            fatal_error("SQL error: %s\n", sqlite3_a3d_errmsg(g.db));
        }
        if (sqlite3_a3d_step(pStmt) == SQLITE_ROW) {
            const char* z = (const char*)sqlite3_a3d_column_text(pStmt, 0);
            if (z) zResult = sqlite3_a3d_mprintf("%s", z);
        }
        sqlite3_a3d_finalize(pStmt);
    }
    sqlite3_a3d_free(zSql);
    speedtest1_shrink_memory();
    return zResult;
...
void speedtest1_prepare(const char* zFormat, ...) {
    ...
    if (g.bSqlOnly) {
        ...
        int rc;
        if (g.pStmt) sqlite3_a3d_finalize(g.pStmt);
        rc = sqlite3_a3d_prepare_v2(g.db, zSql, -1, &g.pStmt, 0);
        if (rc) {
            fatal_error("SQL error: %s\n", sqlite3_a3d_errmsg(g.db));
        }
    }
...
void speedtest1_run(void) {
    ...
    assert(g.pStmt);
    g.nResult = 0;
    while (sqlite3_a3d_step(g.pStmt) == SQLITE_ROW) {
        n = sqlite3_a3d_column_count(g.pStmt);
        for (i = 0; i < n; i++) {
            const char* z = (const char*)sqlite3_a3d_column_text(g.pStmt, i);
            if (z == 0) z = "nil";
            len = (int)strlen(z);
#ifndef SPEEDTEST_OMIT_HASH
            if (g.bVerify) {
                int eType = sqlite3_a3d_column_type(g.pStmt, i);
                ...
                if (eType == SQLITE_FLOAT) {
                    ...
                    ** a floating-point value. */
                    ...
                    int nBlob = sqlite3_a3d_column_bytes(g.pStmt, i);
                    int iBlob;
                    unsigned char zChar[2];
                    const unsigned char* aBlob = reinterpret_cast<const unsigned char *>(sqlite3_a3d_column_blob(g.pStmt, i));
    ...
    if (g.bReprepare) {
        sqlite3_a3d_stmt* pNew;
        sqlite3_a3d_prepare_v2(g.db, sqlite3_a3d_sql(g.pStmt), -1, &pNew, 0);
        sqlite3_a3d_finalize(g.pStmt);
        g.pStmt = pNew;
    }
    else
#endif
    {
        sqlite3_a3d_reset(g.pStmt);
    }
    speedtest1_shrink_memory();
}

#ifndef SQLITE_OMIT_DEPRECATED
/* The sqlite3_a3d_trace() callback function */
...
static void randomFunc(
    sqlite3_a3d_context* context,
    int NotUsed,
    sqlite3_a3d_value** NotUsed2
) {
    sqlite3_a3d_result_int64(context, (sqlite3_a3d_int64)speedtest1_random());
}
...
static void groupAppend(struct groupConcat* p, const char* z, int n) {
    if (p->nUsed + n >= p->nAlloc) {
        ...
        char* z2 = sqlite3_a3d_realloc(p->z, n2);
        if (z2 == 0) return;
        p->z = z2;
        p->nAlloc = n2;
    }
...
static void groupStep(
    ...
    if (sqlite3_a3d_value_type(argv[0]) == SQLITE_NULL) return;
    p = (struct groupConcat*)sqlite3_a3d_aggregate_context(context, sizeof(*p));

    if (p) {
        int firstTerm = p->nUsed == 0;
        if (!firstTerm) {
            if (argc == 2) {
                zSep = (char*)sqlite3_a3d_value_text(argv[1]);
                nSep = sqlite3_a3d_value_bytes(argv[1]);
            }
            else {
                zSep = ",";
                nSep = 1;
        ...
        zVal = (char*)sqlite3_a3d_value_text(argv[0]);
        nVal = sqlite3_a3d_value_bytes(argv[0]);
        if (zVal) groupAppend(p, zVal, nVal);
    }
}
static void groupFinal(sqlite3_a3d_context* context) {
    struct groupConcat* p;
    p = sqlite3_a3d_aggregate_context(context, 0);
    if (p && p->z) {
        p->z[p->nUsed] = 0;
        sqlite3_a3d_result_text(context, p->z, p->nUsed, sqlite3_a3d_free);
    }
}
#endif
...
void testset_main(void) {
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "INSERT INTO t1 VALUES(" + std::to_string((sqlite3_a3d_int64)x1) + ',' + std::to_string(i) + ",\"" + std::to_string(*zNum) + "\");"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_int64(g.pStmt, 1, (sqlite3_a3d_int64)x1);
        sqlite3_a3d_bind_int(g.pStmt, 2, i);
        sqlite3_a3d_bind_text(g.pStmt, 3, zNum, -1, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    ...
    for (i = 1; i <= n; i++) {
        ...
        speedtest1_numbername(x1, zNum, sizeof(zNum));
        if (!wrapper.ExecStatement((
            "INSERT INTO t2 VALUES(" + std::to_string(i) + ',' + std::to_string((sqlite3_a3d_int64)x1) + ",\"" + std::to_string(*zNum) + "\");"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_int(g.pStmt, 1, i);
        sqlite3_a3d_bind_int64(g.pStmt, 2, (sqlite3_a3d_int64)x1);
        sqlite3_a3d_bind_text(g.pStmt, 3, zNum, -1, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    ...
    for (i = 1; i <= n; i++) {
        ...
        speedtest1_numbername(x1, zNum, sizeof(zNum));
        if (!wrapper.ExecStatement((
            "INSERT INTO t3 VALUES(" + std::to_string(i) + ',' + std::to_string((sqlite3_a3d_int64)x1) + ",\"" + std::to_string(*zNum) + "\");"
            ).c_str()))
        {
            printf("fail\n");
            break;
...
#if SQLITE_VERSION_NUMBER<3005004
    sqlite3_a3d_create_function(g.db, "group_concat", 1, SQLITE_UTF8, 0,
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT count(*), avg(b), sum(length(c)), group_concat(c) FROM t1 WHERE b BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_int(g.pStmt, 1, x1);
        sqlite3_a3d_bind_int(g.pStmt, 2, x2);
        speedtest1_run();
        */
    }
    speedtest1_exec("COMMIT");
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT count(*), avg(b), sum(length(c)), group_concat(c) FROM t1 WHERE c LIKE \"" + std::string(zNum) + "\";"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_text(g.pStmt, 1, zNum, len + 1, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    speedtest1_exec("COMMIT");
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT a, b, c FROM t1 WHERE c LIKE \"" + std::string(zNum) + "\" ORDER BY a;"
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT a, b, c FROM t1 WHERE c LIKE \"" + std::string(zNum) +  "\" ORDER BY a LIMIT 10;"
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT count(*), avg(b), sum(length(c)), group_concat(a) FROM t1 WHERE b BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT count(*), avg(b), sum(length(c)), group_concat(a) FROM t2 WHERE a BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT count(*), avg(b), sum(length(c)), group_concat(a) FROM t1 WHERE c BETWEEN \"" + std::string(zNum) + "\" AND ( \"" + std::string(zNum) + "\" ||'~');"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_text(g.pStmt, 1, zNum, len, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    speedtest1_exec("COMMIT");
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        x2 = speedtest1_random() % 10 + sz / 5000 + x1;
        if (!wrapper.ExecStatement((
            "UPDATE t2 SET d=b*2 WHERE b BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        if (!wrapper.ExecStatement((
            "UPDATE t2 SET d=b*3 WHERE a= " + std::to_string(x1) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        ...
        x2 = speedtest1_random() % 10 + sz / 5000 + x1;
        if (!wrapper.ExecStatement((
            "DELETE FROM t2 WHERE b BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= n; i++) {
        x1 = speedtest1_random() % sz + 1;
        if (!wrapper.ExecStatement((
            "DELETE FROM t3 WHERE a=" + std::to_string(x1) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    */
    for (i = 1; i <= 100; i++) {
        ...
        if (!wrapper.ExecStatement((
            "SELECT t1.c FROM t1, t2, t3, t4 WHERE t4.a BETWEEN " + std::to_string(x1) + " AND " + std::to_string(x2) + " AND t3.a=t4.b AND t2.a=t3.b AND t1.c=t2.c"
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    sqlite3_a3d_bind_int(g.pStmt, 1, est_square_root(g.szTest) * 50);
    speedtest1_run();
    */
    if (!wrapper.ExecStatement((
        "SELECT sum(a), max(c), avg((SELECT a FROM t2 WHERE 5+t2.b=t1.b) AND rowid<" + std::to_string(est_square_root(g.szTest)) + "), max(c) FROM t1 WHERE rowid<?1;"
        ).c_str()))
    {
        printf("fail\n");
        return;
    ...
    /*
    speedtest1_prepare(
    "REPLACE INTO t5 VALUES(?1,?2); --  %d times"
    , n);
    */
    for (i = 1; i <= n; i++) {
        x1 = swizzle(i, maxb);
        ...
        if (!wrapper.ExecStatement((
            "REPLACE INTO t5 VALUES(" + std::to_string((sqlite3_a3d_int64)x1) + ",\"" + std::string(zNum) + "\");"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_int(g.pStmt, 1, (sqlite3_a3d_int64)x1);
        sqlite3_a3d_bind_text(g.pStmt, 2, zNum, -1, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    speedtest1_exec("COMMIT");
    ...
    /*
    speedtest1_prepare(
        "SELECT b FROM t5 WHERE a=?1; --  %d times"
        , n);
        */
    for (i = 1; i <= n; i++) {
        x1 = swizzle(i, maxb);
        if (!wrapper.ExecStatement((
            "SELECT b FROM t5 WHERE a=" + std::to_string((sqlite3_a3d_int64)x1) + ';'
            ).c_str()))
        {
            printf("fail\n");
            break;
    ...
    speedtest1_exec("CREATE%s TABLE t6(a TEXT PRIMARY KEY, b %s)%s;",
        isTemp(9), g.zNN,
        sqlite3_a3d_libversion_number() >= 3008002 ? "WITHOUT ROWID" : "");
    /*
    speedtest1_prepare(
    "REPLACE INTO t6 VALUES(?1,?2); --  %d times", n);
    */
    for (i = 1; i <= n; i++) {
        x1 = swizzle(i, maxb);
        speedtest1_numbername(x1, zNum, sizeof(zNum));
        if (!wrapper.ExecStatement((
            "REPLACE INTO t6 VALUES(\"" + std::string(zNum) + "\"," + std::to_string(i) + ");"
            ).c_str()))
        {
            printf("fail\n");
            break;
        ...
        /*
        sqlite3_a3d_bind_int(g.pStmt, 2, i);
        sqlite3_a3d_bind_text(g.pStmt, 1, zNum, -1, SQLITE_STATIC);
        speedtest1_run();
        */
    }
    speedtest1_exec("COMMIT");
    ...
    /*
    speedtest1_prepare(
    "SELECT b FROM t6 WHERE a=?1; --  %d times"
    , n);
    */
    for (i = 1; i <= n; i++) {
        x1 = swizzle(i, maxb);
        speedtest1_numbername(x1, zNum, sizeof(zNum));
        if (!wrapper.ExecStatement((
            "SELECT b FROM t6 WHERE a=\"" + std::string(zNum) + "\";"
            ).c_str()))
        {
            printf("fail\n");
            break;
...
void testset_cte(void) {
    ...
    sqlite3_a3d_bind_text(g.pStmt, 1, zPuz, -1, SQLITE_STATIC);
    speedtest1_run();
    speedtest1_end_test();
    ...
    sqlite3_a3d_bind_double(g.pStmt, 1, rSpacing * .05);
    sqlite3_a3d_bind_double(g.pStmt, 2, rSpacing);
    speedtest1_run();
    speedtest1_end_test();
...
void speedtest1_random_ascii_fp(char* zFP) {
    ...
    if (z < 0) z = -z;
    y /= 10;
    sqlite3_a3d_snprintf(100, zFP, "%d.%de%d", y, z, x % 200);
}
...
void testset_fp(void) {
    ...
    for (i = 1; i <= n; i++) {
        ...
        sqlite3_a3d_bind_text(g.pStmt, 1, zFP1, -1, SQLITE_STATIC);
        sqlite3_a3d_bind_text(g.pStmt, 2, zFP2, -1, SQLITE_STATIC);
        speedtest1_run();
    }
    speedtest1_exec("COMMIT");
    speedtest1_end_test();
...
void testset_rtree(int p1, int p2) {
    ...
    unsigned iStep;
    unsigned mxRowid;
    int* aCheck = sqlite3_a3d_malloc(sizeof(int) * g.szTest * 500);
    ...
    for (i = 1; i <= n; i++) {
        ...
        sqlite3_a3d_bind_int(g.pStmt, 1, i);
        sqlite3_a3d_bind_int(g.pStmt, 2, x0);
        sqlite3_a3d_bind_int(g.pStmt, 3, x1);
        sqlite3_a3d_bind_int(g.pStmt, 4, y0);
        sqlite3_a3d_bind_int(g.pStmt, 5, y1);
        sqlite3_a3d_bind_int(g.pStmt, 6, z0);
        sqlite3_a3d_bind_int(g.pStmt, 7, z1);
        speedtest1_run();
    }
    speedtest1_exec("COMMIT");
    speedtest1_end_test();
    ...
    iStep = mxCoord / n;
    for (i = 0; i < n; i++) {
        sqlite3_a3d_bind_int(g.pStmt, 1, i * iStep);
        sqlite3_a3d_bind_int(g.pStmt, 2, (i + 1) * iStep);
        speedtest1_run();
    ...
    sqlite3_a3d_rtree_geometry_callback(g.db, "xslice", xsliceGeometryCallback, 0);
    ...
    for (i = 1; i <= n; i++) {
        sqlite3_a3d_bind_int(g.pStmt, 1, (i * 251) % mxRowid + 1);
        speedtest1_run();
    }
    speedtest1_end_test();
    ...
    for (i = 1; i <= n; i++) {
        sqlite3_a3d_bind_int(g.pStmt, 1, (i * 257) % mxRowid + 1);
        speedtest1_run();
    }
    speedtest1_end_test();
...
void testset_orm(void) {
    ...
    for (i = 0; i < n; i++) {
        ...
        len = (int)strlen(zNum);
        sqlite3_a3d_bind_int(g.pStmt, 1, i ^ 0xf);
        for (j = 0; zType[j]; j++) {
            switch (zType[j]) {
            case 'I':
            case 'T':
                sqlite3_a3d_bind_int64(g.pStmt, j + 2, x1);
                break;
            case 'F':
                sqlite3_a3d_bind_double(g.pStmt, j + 2, (double)x1);
                break;
            case 'V':
            case 'B':
                sqlite3_a3d_bind_text64(g.pStmt, j + 2, zNum, len,
...
void testset_trigger(void) {
    ...
    for (jj = 1; jj <= 3; jj++) {
        ...
        for (ii = 0; ii < NROW2; ii += 3) {
            sqlite3_a3d_bind_int(g.pStmt, 1, ii * 3);
            speedtest1_run();
        }
    }
    speedtest1_end_test();
    ...
    for (jj = 0; jj < NROW2; jj++) {
        sqlite3_a3d_bind_int(g.pStmt, 1, jj * 3);
        speedtest1_run();
    }
    speedtest1_end_test();
    ...
    for (jj = 0; jj < NROW2; jj++) {
        sqlite3_a3d_bind_int(g.pStmt, 1, jj);
        speedtest1_run();
    }
    speedtest1_exec("COMMIT");
    speedtest1_end_test();
    ...
    */
    ...
    for (jj = 1; jj <= NROW2 * 2; jj += 2) {
        ...
        sqlite3_a3d_bind_int(g.pStmt, 1, jj * 2);
        sqlite3_a3d_bind_text(g.pStmt, 2, zNum, -1, SQLITE_STATIC);
        sqlite3_a3d_bind_int(g.pStmt, 3, jj);
        speedtest1_run();
    }
    speedtest1_end_test();
...
static void displayLinuxIoStats(FILE* out) {
    FILE* in;
    char z[200];
    sqlite3_a3d_snprintf(sizeof(z), z, "/proc/%d/io", getpid());
    in = fopen(z, "rb");
    if (in == 0) return;
...
#  define sqlite3_a3d_sourceid(X) "(before 3.6.18)"
#endif
...
int main(int argc, char** argv) {
    ...
    printf("-- Speedtest1 for SQLite %s %.48s\n",
        sqlite3_a3d_libversion(), sqlite3_a3d_sourceid());
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (z[0] == '-') {
            ...
            if (strcmp(z, "autovacuum") == 0) {
                ...
                sqlite3_a3d_config(SQLITE_CONFIG_MULTITHREAD);
            }
            else if (strcmp(z, "nomemstat") == 0) {
                sqlite3_a3d_config(SQLITE_CONFIG_MEMSTATUS, 0);
#endif
#if SQLITE_VERSION_NUMBER>=3007017
            }
                ...
                sqlite3_a3d_config(SQLITE_CONFIG_SERIALIZED);
            }
            else if (strcmp(z, "singlethread") == 0) {
                sqlite3_a3d_config(SQLITE_CONFIG_SINGLETHREAD);
#endif
            }
...
#if SQLITE_VERSION_NUMBER>=3006001
    printf("--> SQLITE_VERSION_NUMBER>=3006001\n");
    if (nHeap > 0) {
        printf("--> nHeap=(%d)\n", nHeap);
        pHeap = malloc(nHeap);
        if (pHeap == 0) fatal_error("cannot allocate %d-byte heap\n", nHeap);
        rc = sqlite3_a3d_config(SQLITE_CONFIG_HEAP, pHeap, nHeap, mnHeap);
        if (rc) fatal_error("heap configuration failed: %d\n", rc);
    }
    if (doPCache) {
        printf("--> doPCache\n");
        if (nPCache > 0 && szPCache > 0) {
            pPCache = malloc(nPCache * (sqlite3_a3d_int64)szPCache);
            if (pPCache == 0) fatal_error("cannot allocate %lld-byte pcache\n",
                nPCache * (sqlite3_a3d_int64)szPCache);
        }
        rc = sqlite3_a3d_config(SQLITE_CONFIG_PAGECACHE, pPCache, szPCache, nPCache);
        if (rc) fatal_error("pcache configuration failed: %d\n", rc);
    }
    if (nLook >= 0) {
        printf("--> nLook=(%d)\n", nLook);
        sqlite3_a3d_config(SQLITE_CONFIG_LOOKASIDE, 0, 0);
    }
#endif
    sqlite3_a3d_initialize();

    /* Open the database and the input file */
    printf("--> zDbName (%s)\n", zDbName);
    if (sqlite3_a3d_open(memDb ? ":memory:" : zDbName, &g.db)) {
        fatal_error("Cannot open database file: %s\n", zDbName);
    }
#if SQLITE_VERSION_NUMBER>=3006001
    printf("--> SQLITE_VERSION_NUMBER>=3006001 for the second time\n");
    if (nLook > 0 && szLook > 0) {
        printf("--> nLook=(%d), szLook=(%d)\n", nLook, szLook);
        pLook = malloc(nLook * szLook);
        rc = sqlite3_a3d_db_config(g.db, SQLITE_DBCONFIG_LOOKASIDE, pLook, szLook, nLook);
        if (rc) fatal_error("lookaside configuration failed: %d\n", rc);
    }
#endif

    /* Set database connection options */
    sqlite3_a3d_create_function(g.db, "random", 0, SQLITE_UTF8, 0, randomFunc, 0, 0);
#ifndef SQLITE_OMIT_DEPRECATED
    if (doTrace) sqlite3_a3d_trace(g.db, traceCallback, 0);
#endif
    if (memDb > 0) {
        printf("--> memDb > 0\n");
    ...
    if (noSync) {
        printf("--> synchronous=OFF\n");
        speedtest1_exec("PRAGMA synchronous=OFF");
    }
    if (doExclusive) {
        printf("--> locking_mode=EXCLUSIVE\n");
    ...
    do {
        ...
        if (zTSet[0]) {
            ...
            while (1) {
                ...
                if (zObj == 0) break;
                zSql = sqlite3_a3d_mprintf("DROP TABLE main.\"%w\"", zObj);
                speedtest1_exec(zSql);
                sqlite3_a3d_free(zSql);
                sqlite3_a3d_free(zObj);
            }
            while (1) {
                zObj = speedtest1_once(
    ...
    if (showStats) {
        sqlite3_a3d_exec(g.db, "PRAGMA compile_options", xCompileOptions, 0, 0);
    }
    ...
    ** have been finalized */
    ...
    if (showStats) {
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_USED, &iCur, &iHi, 0);
        printf("-- Lookaside Slots Used:        %d (max %d)\n", iCur, iHi);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_HIT, &iCur, &iHi, 0);
        printf("-- Successful lookasides:       %d\n", iHi);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_SIZE, &iCur, &iHi, 0);
        printf("-- Lookaside size faults:       %d\n", iHi);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_LOOKASIDE_MISS_FULL, &iCur, &iHi, 0);
        printf("-- Lookaside OOM faults:        %d\n", iHi);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_CACHE_USED, &iCur, &iHi, 0);
        printf("-- Pager Heap Usage:            %d bytes\n", iCur);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_CACHE_HIT, &iCur, &iHi, 1);
        printf("-- Page cache hits:             %d\n", iCur);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_CACHE_MISS, &iCur, &iHi, 1);
        printf("-- Page cache misses:           %d\n", iCur);
#if SQLITE_VERSION_NUMBER>=3007012
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_CACHE_WRITE, &iCur, &iHi, 1);
        printf("-- Page cache writes:           %d\n", iCur);
#endif
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_SCHEMA_USED, &iCur, &iHi, 0);
        printf("-- Schema Heap Usage:           %d bytes\n", iCur);
        sqlite3_a3d_db_status(g.db, SQLITE_DBSTATUS_STMT_USED, &iCur, &iHi, 0);
        printf("-- Statement Heap Usage:        %d bytes\n", iCur);
    }
#endif

    sqlite3_a3d_close(g.db);

#if SQLITE_VERSION_NUMBER>=3006001
    /* Global memory usage statistics printed after the database connection
    ** has closed.  Memory usage should be zero at this point. */
    if (showStats) {
        sqlite3_a3d_status(SQLITE_STATUS_MEMORY_USED, &iCur, &iHi, 0);
        printf("-- Memory Used (bytes):         %d (max %d)\n", iCur, iHi);
#if SQLITE_VERSION_NUMBER>=3007000
        sqlite3_a3d_status(SQLITE_STATUS_MALLOC_COUNT, &iCur, &iHi, 0);
        printf("-- Outstanding Allocations:     %d (max %d)\n", iCur, iHi);
#endif
        sqlite3_a3d_status(SQLITE_STATUS_PAGECACHE_OVERFLOW, &iCur, &iHi, 0);
        printf("-- Pcache Overflow Bytes:       %d (max %d)\n", iCur, iHi);
        sqlite3_a3d_status(SQLITE_STATUS_MALLOC_SIZE, &iCur, &iHi, 0);
        printf("-- Largest Allocation:          %d bytes\n", iHi);
        sqlite3_a3d_status(SQLITE_STATUS_PAGECACHE_SIZE, &iCur, &iHi, 0);
